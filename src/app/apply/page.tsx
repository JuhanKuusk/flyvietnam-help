...
function ApplyForm() {
  ...
  const { formatSitePrice, siteConfig } = useSite();
      ...
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="max-w-6xl mx-auto px-2 sm:px-6 lg:px-8 py-3 sm:py-4 flex justify-between items-center">
          <a href="/" className="hidden sm:flex items-center gap-3">
            <Logo size="md" taglineText={t.header?.logoTagline || t.applyHeader?.tagline || "Expedited Processing in 30 min"} siteName={siteConfig?.content.siteName || t.header?.siteName} />
          </a>
          <a href="/" className="sm:hidden text-lg font-bold" style={{ color: '#c41e3a' }}>
            {siteConfig?.content.siteName || t.header?.siteName || "FlyVietnam.Help"}
          </a>
              ...
              <div className="lg:col-span-2 flex gap-4 pt-6">
                ...
                {travelDetails.applicantCount > 1 && currentApplicant < travelDetails.applicantCount - 1 ? (
                  ...
                  <div className="flex-1 flex flex-col gap-4">
                    ...
                    <FlightRiskBlock
                      countryCode={initialDepartingCountry || applicants[0]?.nationality || ""}
                      visaSpeed={selectedVisaSpeed}
                      language={language}
                      airportCode={initialDepartingAirport || undefined}
                    />
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                      ...
                      <div className="h-[200px] overflow-y-scroll p-4 text-xs text-gray-600 dark:text-gray-400 leading-relaxed scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
                        ...
                        <p className="mb-3">{(t.applyForm.termsPreview1Content || `"Provider" - ${siteConfig?.content.siteName || "FlyVietnam.Help"} is an e-commercial or non-government website. "Client" - An individual or company who conducts payment of the Sales Order/Booking. "Terms and Conditions" - An agreement between The Client and The Provider governing the rights, obligations, and responsibilities of both parties.`).replace(/FlyVietnam\.Help/g, siteConfig?.content.siteName || "FlyVietnam.Help")}</p>

                        <p className="font-semibold text-gray-900 dark:text-white mb-2">{t.applyForm.termsPreview2Title || "2. Acceptance of Terms"}</p>
                        <p className="mb-3">{(t.applyForm.termsPreview2Content || `By using our Website and Service, you acknowledge that you have read, understood, and agree to be bound by these Terms and Conditions. You understand that ${siteConfig?.content.siteName || "FlyVietnam.Help"} is an independent visa assistance service and is NOT affiliated with the Vietnamese Government or any governmental body. You agree not to use the website for illegal purposes and to respect all applicable laws.`).replace(/FlyVietnam\.Help/g, siteConfig?.content.siteName || "FlyVietnam.Help")}</p>

                        <p className="font-semibold text-gray-900 dark:text-white mb-2">{t.applyForm.termsPreview3Title || "3. Nature of Our Service"}</p>
                        <p className="mb-3">{(t.applyForm.termsPreview3Content || `${siteConfig?.content.siteName || "FlyVietnam.Help"} provides professional visa application assistance services including document review and verification, application form preparation and submission, 24/7 customer support, application status tracking, and express processing options. We do not issue visas. All visa decisions are made solely by the Vietnam Immigration Department.`).replace(/FlyVietnam\.Help/g, siteConfig?.content.siteName || "FlyVietnam.Help")}</p>
                        ...
                        <p className="mb-3">{t.applyForm.termsPreview10Content || `Email: ${siteConfig?.content.supportEmail || "info@flyvietnam.help"} | WhatsApp: ${siteConfig?.content.whatsappNumber || "+84 70 5549868"} | Address: Park 7 Building, Floor 38, Vinhomes Central Park, 720A, Binh Thanh District, Ho Chi Minh City, Vietnam`}</p>
